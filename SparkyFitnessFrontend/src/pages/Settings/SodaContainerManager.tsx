...
import {
  ...
  useCreateSodaContainerMutation,
  useDeleteSodaContainerMutation,
  useSetPrimarySodaContainerMutation,
} from '@/hooks/Settings/useSodaContainers';
import { useAuth } from '@/hooks/useAuth';

const SodaContainerManager: React.FC = () => {
  const { user } = useAuth();
  ...
  const { data: containers = [] } = useSodaContainersQuery(user?.activeUserId);
  const { mutateAsync: createSodaContainer } = useCreateSodaContainerMutation();
  const { mutateAsync: deleteSodaContainer } = useDeleteSodaContainerMutation();
  const { mutateAsync: setPrimarySodaContainer } =
    useSetPrimarySodaContainerMutation();

  const handleAddContainer = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || volume === '' || servingsPerContainer === '') return;
    await createSodaContainer({
      name,
  ...
  const handleDeleteContainer = async (id: number) => {
    await deleteSodaContainer(id);
    toast({ title: 'Success', description: 'Soda container deleted.' });
  };

  const handleSetPrimary = async (id: number) => {
    await setPrimarySodaContainer(id);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Manage Soda Containers</CardTitle>
      </CardHeader>
      <CardContent>
        <form
...
export default SodaContainerManager;
