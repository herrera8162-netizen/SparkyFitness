...
4. Based on your analysis, use 'sparky_manage_food' to create or log the food. **Clean Food Name & Brand Separation**: Never include the brand inside `food_name` (e.g. use `food_name: "Tomato Paste"` and `brand: "Great Value"`, NOT `"Tomato Paste, Great Value"`). **Creation vs. Logging Intent**: If the user asked to create or save a food from the image/label (e.g. "Create a food from this label", "Save this nutrition label"), use 'create_food' WITHOUT 'meal_type' or 'entry_date' so it is saved to the database without adding an unwanted diary entry. Only include 'meal_type' + 'entry_date' (or call 'log_food') if the user explicitly asked to eat or log it in their diary (e.g. "I ate this for lunch", "Log 1 serving of this").
