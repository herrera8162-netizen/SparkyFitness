...
| :--- | :--- | :--- |
| **Manage Diary**<br>`can_manage_diary` | ✍️ Food entries, exercise entries, water logs, soda logs, daily goal plans, favorite (starred) foods/meals, and the owner's custom food library items (foods, food unit variants, meals, and meal ingredients).<br><br>❌ *The exercise library remains owner-only to write; delegates with `can_manage_diary` can still only read it.* | 📖 Food entries, exercise entries, water logs, soda logs, daily goal plans, favorite (starred) foods/meals, custom food/exercise libraries, and read-only profile/layout/water-container/soda-container settings.<br><br>❌ *Wellness logs (fasting, sleep, mood) and medication logs are completely blocked.* |
| **Manage Check-in**<br>`can_manage_checkin` | ✍️ Weight, body measurements, progress photos, mood logs, fasting status, sleep logs, custom measurement categories/values (e.g. GLP-1 daily check-in), smart scale composition (muscle mass, bone mass, water %), and generic health telemetry (heart rate, HRV, respiration, SpO2, vitals, daily health scores). | 📖 Weight history, progress photos, wellness logs (mood, fasting, sleep), custom measurements, smart scale metrics, generic health telemetry, and read-only profile/layout settings.<br><br>❌ *Diary logs (food, exercise, water) and medication logs are completely blocked.* |
...
Your custom food library, recipes, custom exercises, and medication inventory are only visible to delegates when they have switched their active profile context to your profile. This prevents their personal libraries/cabinets from being cluttered with your items. Delegates with `can_manage_diary` may also edit your foods, food unit variants, meals, and meal ingredients while switched into your profile — the exercise library stays read-only for them.
