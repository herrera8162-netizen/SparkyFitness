...
| :--- | :--- | :--- |
| **Manage Diary**<br>`can_manage_diary` | ✍️ Food entries, exercise entries, water logs, daily goal plans, favorite (starred) foods/meals, and the owner's custom food library items (foods, food unit variants, meals, and meal ingredients).<br><br>❌ *The exercise library remains owner-only to write; delegates with `can_manage_diary` can still only read it.* | 📖 Food entries, exercise entries, water logs, daily goal plans, favorite (starred) foods/meals, custom food/exercise libraries, and read-only profile/layout/water-container settings.<br><br>❌ *Wellness logs (fasting, sleep, mood) and medication logs are completely blocked.* |
