...
- **Description:** Creates a new meal template directly with explicit ingredients (foods or nested sub-meals).
- **Parameters:**
    - `meal_name` (string): The name for the new meal template.
    - `foods` (array of objects): List of ingredients with `food_name` or `food_id`, `quantity`, `unit`, and optional `variant_id` or `child_meal_id`/`child_meal_name`.
    - `description` (string, optional): A description for the meal template.
    - `is_public` (boolean, optional): Whether to share this template publicly.
    - `serving_size` (number, optional): Default serving yield size.
    - `serving_unit` (string, optional): Default serving unit (e.g., 'serving', 'g').
    - `total_servings` (number, optional): Total recipe yield count.
    - `cooked_weight_g` (number, optional): Total finished cooked weight in grams.

### `update_meal_template`
- **Description:** Updates an existing meal template's metadata or replaces its ingredient list.
- **Parameters:**
    - `meal_id` (string, UUID, optional): The ID of the meal template to update (at least one of `meal_id` or `meal_name` required).
    - `meal_name` (string, optional): Current name of the meal template to update.
    - `new_name` (string, optional): New name for the meal template.
    - `description` (string, optional): Updated description.
    - `is_public` (boolean, optional): Updated public sharing status.
    - `serving_size` (number, optional): Updated serving yield size.
    - `serving_unit` (string, optional): Updated serving unit.
    - `total_servings` (number, optional): Updated total recipe yield count.
    - `cooked_weight_g` (number, optional): Updated cooked weight in grams.
    - `foods` (array of objects, optional): Replacement list of ingredients.

### `delete_meal_template`
- **Description:** Deletes a meal template.
- **Parameters:**
    - `meal_id` (string, UUID, optional): The ID of the meal template to delete (at least one of `meal_id` or `meal_name` required).
    - `meal_name` (string, optional): The name of the meal template to delete.
