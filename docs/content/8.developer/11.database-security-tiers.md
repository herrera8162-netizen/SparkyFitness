...
**Exception:** `foods`, `food_variants`, `meals`, and `meal_foods` also permit write access to a delegate with `can_manage_diary` (see rows below) — the food library is delegate-writable while the exercise library remains owner-only.
...
| `foods` | Custom food items created by the user (including `is_cooked` state) | Owner or Delegate with `can_manage_diary` | Delegate with `can_view_food_library`, `can_manage_diary`, or `can_view_reports` |
| `food_variants` | Serving options / sizes for custom foods | Owner or Delegate with `can_manage_diary` | Delegate with `can_view_food_library`, `can_manage_diary`, or `can_view_reports` |
| `meals` | Custom meals created by the user | Owner or Delegate with `can_manage_diary` | Delegate with `can_view_food_library`, `can_manage_diary`, or `can_view_reports` |
| `meal_foods` | Ingredients assigned inside a custom meal — either a food (`food_id`) or a linked reusable sub-meal (`child_meal_id`, `item_type='meal'`). Writing a linked-meal row requires the caller to also have library access to the referenced sub-meal, in addition to owning (or having `can_manage_diary` access to) the parent meal. | Owner or Delegate with `can_manage_diary` (and the referenced food/sub-meal must be accessible to them) | Delegate with `can_view_food_library`, `can_manage_diary`, or `can_view_reports` |
...
| `user_soda_containers` | Configured soda/sugary-drink container sizes (e.g. 355ml can) | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
...
| `soda_intake_entries` | Individual logged soda/sugary-drink servings for the day (daily total is computed with `SUM(soda_ml)`, no separate rollup table) | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
