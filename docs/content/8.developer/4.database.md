...
| Table | Purpose |
|-------|---------|
| `foods` | Custom food items created by user (includes `is_cooked` property for nutritional inference) |
...
| `soda_intake_entries` | Individual logged soda/sugary-drink servings (daily total computed via `SUM`) |
| `user_soda_containers` | Configured soda container sizes |
